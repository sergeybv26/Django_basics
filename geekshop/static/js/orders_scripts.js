window.onload = function () {
    ...
    let table = document.getElementByClassName('table');

    $('.order_form').on('change', 'select', function () {
       var target = event.target;
       orderitem_num = parseInt(target.name.replace('orderitems-', '').replace('-product', ''));
       $.ajax({
            url: "product/added/" + target.value + "/",
            success: function (data) {
                console.log('ajax')
                let price_cell = table.getElementsByTagName('tr')[orderitem_num].getElementsByTagName('td')[3]
                console.log(price_cell)
                $('.').html(data.result);
            },
       });
    });
